...
pub trait Methods: Plain {
    ...
    #[inline]
    fn reinterpret<T>(&self) -> &T where T: Methods + ?Sized {
        self::reinterpret(self)
    }
    
    #[inline]
    fn reinterpret_with_len<T>(&self, max_items: usize) -> &[T] where T: Methods {
        self::reinterpret_with_len(self, max_items)
    }
    
    #[inline]
    fn reintepret_mut<T>(&mut self) -> &mut T where T: Methods + ?Sized {
        self::reinterpret_mut(self)
    }
    
    #[inline]
    fn reintepret_mut_with_len<T>(&mut self, max_items: usize) -> &mut [T] where T: Methods {
...
impl<S> Methods for S where S: Plain + Sized + Copy {

    #[inline]
    unsafe fn from_raw_bytes<'a>(ptr: *const u8, bytes: usize) -> &'a S {
        assert!(mem::size_of::<S>() <= bytes, "plain::reinterpret(): input is too short for target type");
...
pub fn reinterpret_mut<T, S>(s: &mut S) -> &mut T
    where S: Plain + ?Sized, T: Methods + ?Sized {
